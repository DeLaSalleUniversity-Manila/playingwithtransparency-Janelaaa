package angeles.janela.assumpta.transparency;
...
public class CameraActivity extends Activity implements TextureView.SurfaceTextureListener, Camera.PreviewCallback {
    ...
    private void setupSeekbar(){
        ...
        seekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                ...
                label.setText("  opacity = " + opacity);
                label.setTextColor(Color.RED);

                imViewA.setAlpha((float)(opacity/256.0));
            }
    ...
    public native void process(Bitmap pTarget, byte[] pSource, int opacity);

    private Runnable DoImageProcessing = new Runnable() {
        public void run() {
            bProcessing = true;
            process(imageA, videoSource, opacity);
            imViewA.invalidate();
